...
const Library = () => {
  ...
  const filteredPrompts = prompts.filter(prompt => {
    ...
    const matchesSearch = searchQuery === "" || prompt.title.toLowerCase().includes(searchQuery.toLowerCase()) || prompt.description.toLowerCase().includes(searchQuery.toLowerCase()) || prompt.category.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  return <Layout>
      <div className="space-y-8">
        <div className="flex justify-between items-center">
          ...
          <PromptFilters filter={filter} searchQuery={searchQuery} onFilterChange={setFilter} onSearchChange={setSearchQuery} />
        </div>

        <PromptTable prompts={filteredPrompts} isAdmin={isAdmin} onCustomize={handleCustomizePrompt} onCopy={copyToClipboard} onDelete={prompt => setDeletePromptId(prompt.id)} />
      </div>

      <CustomPromptWizard basePrompt={selectedPrompt} isOpen={isWizardOpen} onClose={() => {
