...
import { processDocumentWithGraphlit, storeDocumentNodes } from "../_shared/graphlit.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    if (!graphlitApiKey) throw new Error('GRAPHLIT_TESTING_API_KEY environment variable is not set');
    
    // Log initialization status
    console.log("Edge function initialization:", {
      hasSupabaseUrl: !!supabaseUrl,
      hasSupabaseKey: !!supabaseKey,
      hasGraphlitKey: !!graphlitApiKey,
      method: req.method,
      url: req.url
    });
    ...
    // Process document using Graphlit
    console.log("Processing document with Graphlit...");
    const fileName = metadata.original_filename || `${title}.${fileType}`;
    
    // Detailed logging before Graphlit API call
    console.log("Preparing Graphlit request:", {
      fileName: fileName,
      fileType: fileType,
      processingOptions: {
      ...
      hasApiKey: !!graphlitApiKey,
      contentLength: content?.length,
      isBase64: isBase64
    });
    
    try {
      const processedDoc = await processDocumentWithGraphlit(
        content,
        fileName,
        fileType,
        {
      ...
      console.log(`Graphlit document ID: ${processedDoc.document_id}`);
      ...
      // Update document with processed content, Graphlit document ID and metadata
      await supabase
        .from('documents')
        .update({
          content: processedDoc.chunks.map(chunk => chunk.text).join("\n\n"),
          graphlit_doc_id: processedDoc.document_id,
          metadata: {
            ...document.metadata,
      ...
      const enrichedChunks = processedDoc.chunks.map((chunk, index) => ({
        ...
        graphlit_chunk_id: chunk.chunk_id
      }));
      ...
      console.error("Error during Graphlit processing:", {
